...
int32_t float_to_q31(float angle_rad) {
    float normalized = angle_rad * 0.318309886f; // Faster than dividing by PI
    if (normalized >= 1.0f) return 2147483647;
    if (normalized < -1.0f) return -2147483648;
    return static_cast<int32_t>(normalized * 2147483648.0f);
}

float q31_to_float(int32_t q31_val) {
    return static_cast<float>(q31_val) / 2147483648.0f;
}
