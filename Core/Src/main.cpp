...
static void process_command(const char* cmd) {
    if (strcmp(cmd, "start") == 0) {
      ...
      usb_printf("Stopped\r\n");
        ...
        /* "foc <rpm>"  — start FOC at the given speed */
        int rpm;
        if (sscanf(cmd + 4, "%d", &rpm) == 1) {
            foc_reset(&foc_state);
            foc_state.target_rpm = (float)rpm;
            motorPWM.start();   /* enable all 3 complementary pairs */
            control_mode = MotorControlMode::MOTOR_FOC_LINEAR;
            usb_printf("FOC started  target=%d RPM\r\n", rpm);
        } else {
            usb_printf("Usage: foc <rpm>\r\n");
        }
    } else if (strncmp(cmd, "rpm ", 4) == 0) {
        /* "rpm <value>"  — update speed setpoint while FOC is running */
        int rpm;
        if (sscanf(cmd + 4, "%d", &rpm) == 1) {
            foc_state.target_rpm = (float)rpm;
            usb_printf("Target RPM set to %d\r\n", rpm);
        } else {
            usb_printf("Usage: rpm <value>\r\n");
        }
    } else if (strcmp(cmd, "status") == 0) {
        usb_printf("RPM=%.1f  Id=%.3f  Iq=%.3f  Vd=%.2f  Vq=%.2f  Vdc=%.2f  fault=%d\r\n",
                   foc_state.rpm,
        ...
        if (sscanf(cmd + 6, "%d", &speed) == 1) {
            ...
            usb_printf("Invalid speed\r\n");
        }
    } else {
        usb_printf("Unknown command\r\n");
        usb_printf("Commands: start | stop | foc <rpm> | rpm <rpm> | status | speed <duty>\r\n");
    }
}
...
/*
 * foc_isr_tick — called from the TIM8 update ISR at 20 kHz.
 *
 * Reads ADC + encoder, applies current offsets, calls foc_run(),
 * and writes resulting duty cycles to the inverter.
 * Must complete within one PWM period (50 µs).
 */
static void foc_isr_tick(void)
{
    /* Guard: fault latched — disable outputs and stop */
    if (foc_state.fault) {
        motorPWM.stop();
        control_mode = MotorControlMode::MOTOR_STOP;
        return;
    ...
    /* ------------------------------------------------------------------
     * 1. Read ADC (latest DMA sample, interrupt-safe via NDTR check)
     *
     * ADC channel mapping (from board / timer4IRQ):
     *   ADC1[0] = Ia   (16-bit, gain=50, shunt=13 mΩ)
     *   ADC1[1] = Vbc  (voltage sense)
     *   ADC1[2] = Vbatt (DC bus voltage)
     *   ADC2[0] = Ib   (16-bit)
     *   ADC2[1] = Vab
     *   ADC3[0] = Ic   (12-bit, 4096 counts)
     *   ADC3[1] = Ibatt
     * ------------------------------------------------------------------ */
    uint16_t adc1_raw[3];
    uint16_t adc2_raw[2];
    ...
    float Ia = adcToCurrent(adc1_raw[0], 3.3f, 65536, 50.0f, 0.0f, 0.013f) - FOC_CURRENT_OFFSET_A;
    float Ib = adcToCurrent(adc2_raw[0], 3.3f, 65536, 50.0f, 0.0f, 0.013f) - FOC_CURRENT_OFFSET_B;
    float Ic = adcToCurrent(adc3_raw[0], 3.3f,  4096, 50.0f, 0.0f, 0.013f) - FOC_CURRENT_OFFSET_C;

    float Vdc = adcToVoltage(adc1_raw[2], 3.3f, 65536, 0.130435f, 0.0f);

    /* Guard: Vdc=0 would cause divide-by-zero inside SVPWM */
    if (Vdc < 1.0f) return;

    /* ------------------------------------------------------------------
     * 2. Read encoder → electrical angle and mechanical speed
     *   theta_e  = theta_mech * pole_pairs  (wrapped 0…2π)
     *   omega_m  = mechanical rad/s
     * ------------------------------------------------------------------ */
    float theta_mech = encoder.getPos_rad();
    float theta_e    = fmodf(theta_mech * (float)FOC_POLE_PAIRS, 2.0f * M_PI);
